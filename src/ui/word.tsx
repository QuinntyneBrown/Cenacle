...
import { JournalEditor, JournalService, NetworkGuard, ReflectionFeedback, ReflectionService } from "../word/journal";
...
export function ScripturePanel({ compact = false }: { compact?: boolean }) {
  ...
  const [result, setResult] = useState<ReturnType<PassageMatcher["surface"]> | null>(null);
  ...
  const another = () => {
    ...
    const next = matcher.surface(theme, result?.passage ? [...excluded, result.passage.id] : excluded);
    setResult(next);
    setSaved(false);
  };
  ...
  return <div className={`stack gap-5 ${compact ? "" : "container container-narrow section"}`}>
    {!compact && <div className="stack gap-3"><span className="eyebrow">Subsystem B · Word</span><h1 className="h1">What's on your heart?</h1><p className="lede">Name a theme in plain words and an existing passage surfaces from the index on this device.</p></div>}
    <form className="field" onSubmit={(event) => { event.preventDefault(); surface(); }}>
      <label className="label" htmlFor={compact ? "theme-compact" : "theme"}>Name a theme</label>
      <div className="cluster gap-3"><input className="input" id={compact ? "theme-compact" : "theme"} value={theme} maxLength={80} onChange={(event) => setTheme(event.target.value)} placeholder="fear · gratitude · waiting…"/><button className="btn btn--primary" type="submit"><Icon name="sparkle"/>Surface a passage</button></div>
      <p className="hint">Or start from one of these:</p>
      <div className="cluster gap-2">{quickThemes.map((item) => <button className="pill" type="button" key={item} onClick={() => surface(item)}>{item}</button>)}</div>
    </form>
    {result?.matched && result.passage && <div className="verse-card stack gap-4"><span className="tag">Surfaced for “{theme}”</span><blockquote className="verse sacred">{result.passage.text}<span className="ref">{result.passage.reference}</span></blockquote><p className="small muted">Why this surfaced — matched against a local verse index. Passages are retrieved, never generated.</p><div className="verse-actions"><button className="btn btn--ghost" onClick={another}><Icon name="refresh"/>Surface another</button><a className="btn btn--quiet" href={result.passage.contextUrl} target="_blank" rel="noreferrer"><Icon name="word"/>Read in context</a><button className="btn btn--quiet" onClick={() => save(result.passage!)}>{saved ? "Saved on this device" : "Save"}</button></div></div>}
    {result && !result.matched && <div className="banner banner--warn"><Icon name="alert"/><div><p className="banner__title">No passage matched that theme</p><p className="small muted">Try a plainer word. No reference has been invented.</p><div className="cluster gap-2 mt-3">{result.suggestions.map((item) => <button type="button" className="pill" onClick={() => surface(item)} key={item}>{item}</button>)}</div></div></div>}
    <div className="banner banner--sage"><Icon name="shield"/><div><p className="banner__title">Grounded in a local verse index</p><p className="small muted">Nothing you type leaves this device.</p></div></div>
    {recent.length > 0 && <section><h2 className="h3">Recent themes</h2><div className="cluster gap-2 mt-3">{recent.map((item) => <button type="button" className="pill" onClick={() => surface(item)} key={item}>{item}</button>)}</div></section>}
    <PrivacySeal/>
  </div>;
}

export function ScripturePage() { return <AppShell><ScripturePanel/></AppShell>; }
...
export function JournalPanel({ compact = false }: { compact?: boolean }) {
  ...
  const feedback = (choice: ReflectionFeedback) => {
    if (current && reflection) service.recordFeedback(current.id, reflection, choice);
    setReflection(null);
    setEntries(service.listEarlier());
    setMessage(choice === ReflectionFeedback.KeepWithEntry ? "Reflection kept with the entry on this device." : "Reflection dismissed. Nothing was sent.");
  };

  return <div className={`stack gap-5 ${compact ? "" : "container container-narrow section"}`}>
    {!compact && <div><span className="eyebrow">Private companion</span><h1 className="h1 mt-3">Lament journal</h1><p className="lede mt-3">Write without performing. Your words stay on this device.</p></div>}
...
export function JournalPage() { return <AppShell night><JournalPanel/></AppShell>; }
