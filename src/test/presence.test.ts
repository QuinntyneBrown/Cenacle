...
import { CodecPreference, ParticipantRole, ReactionKind, type Room } from "../core/types";
import { deserializeMediaPacket, serializeMediaPacket, type EncodedMediaPacket } from "../media/codec";
import { RoomClock } from "../media/room-transport";
import { GatheringSetup, JoinConfig, RoomResolver } from "../presence/controllers";
...
describe("Presence entry", () => {
  ...
  it("normalizes both plain codes and invite links", () => {
    ...
    expect(sanitizer.parseRoomReference("https://cenacle.test/r/abc234")).toBe("ABC234");
  ...
  it("resolves links through the same room lookup and preserves pre-join state", async () => {
    ...
    await expect(resolver.resolve("https://cenacle.test/r/abc234")).resolves.toEqual({ code: "ABC234", status: "open" });
    expect(api.resolve).toHaveBeenCalledWith("ABC234");
    expect(new JoinConfig(" Quinn ", false, false)).toMatchObject({ micEnabled: false, cameraEnabled: false });
...
describe("Presence media", () => {
  it("round-trips encoded H.264 and VP9 packet metadata and bytes", () => {
    for (const codec of ["avc1.42E01E", "vp09.00.10.08"]) {
      ...
      expect(deserializeMediaPacket(serializeMediaPacket(packet))).toEqual(packet);
    }
  ...
  it("retains a non-mutating live latency sample and enforces the 400 ms budget", () => {
    ...
    expect(new LatencyBudget().shouldDrop({ captureTs: 0, renderTs: 601, glassToGlassMs: 601 })).toBe(true);
  });
...
describe("Presence controls and reactions", () => {
  it("marks host, self, mute, camera, and speaking state in the roster snapshot", () => {
    ...
    store.applyRelayed({ participantId: "host", isMuted: true, isCameraOff: true, isSpeaking: true });
    expect(store.snapshot()).toMatchObject({
      presentCount: 1,
      participants: [{ role: ParticipantRole.Host, isSelf: true, isMuted: true, isCameraOff: true, isSpeaking: true }]
    });
  });
  ...
  it("sends reactions as datagrams no faster than the client interval", async () => {
    ...
    await expect(sender.send(ReactionKind.RaisedHand, 1_500)).resolves.toBe(false);
    await expect(sender.send(ReactionKind.RaisedHand, 1_750)).resolves.toBe(true);
  ...
  it("maintains a rolling 60-second reaction count that ages down to zero", () => {
    ...
    counter.record({ kind: ReactionKind.RaisedHand, senderId: "b", sentAt: 20_000 });
